...
- [API가이드 - 마이데이터 인증 API 규격](#api가이드---마이데이터-인증-api-규격) ✅ NEW
...
│   └ 통합인증 API                 │
├─────────────────────────────────┤
│ 마이데이터 지원 API 규격(support)│ ← 다음 구현 예정
├─────────────────────────────────┤
...
http.authorizeHttpRequests(auth -> auth
    ...
    .requestMatchers("/support-api/**").permitAll()  // 다음 구현 시 추가 필요
